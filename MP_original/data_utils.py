...
def alt_compute_training_batches_w2v(gt_file, dbn, qbn, batch_size, padding_value, fold_idx, n_iter_per_query=500,
    ...
    if not os.path.isfile('qn_rd_nrd_pairs_w2v' + str(fold_idx)):
        rd_b_qry = {}
        nrd_by_qry = {}
        ...
        util.save_model(qn_rd_nrd_pairs, 'qn_rd_nrd_pairs_w2v' + str(fold_idx))
    else:
        qn_rd_nrd_pairs = util.load_model('qn_rd_nrd_pairs_w2v' + str(fold_idx))
...
def pre_compute_test_fd_w2v_gc(qbn, dbn, w, run_to_rerank, max_q_len, max_d_len, fold, output_folder):
    ...
    if not os.path.isfile(test_fd_fp):
        ...
        for q_name in tqdm(qbn.keys()):
            ...
            util.save_model(([len(qbn[q_name])] * len(docs), d_lengths, d_names, q_name, sim_m),
                            test_fd_fp + '_qn=' + q_name)
