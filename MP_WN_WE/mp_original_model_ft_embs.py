...
def run():
    ...
    run_name = 'original_model_mp_FT_embs'
    batch_size = 64
    n_epochs = 20
    n_folds = 5
    max_patience = 5
    ...
    wn2vec_model = None # gensim.models.KeyedVectors.load_word2vec_format('../data/wn_embeddings/wn2vec.txt')
    ...
    for fold_index in range(len(folds)):
        ...
        with tf.Session(config=sess_config) as sess:
            ...
            training_q_b_name = {training_q_names[i]: encoded_qbn[training_q_names[i]] for i in
                                 range(len(training_q_names)) if training_q_names[i] not in validation_q_names}
            ...
            for epoch in range(n_epochs):
                ...
                print('BEST valid map in fold=%2.4f, at epoch=%d, iter=%d' % (max_map, best_epoch, best_iter))
